...
if __name__ == '__main__':
    ...
    parser.add_argument("--audio_path", type=Path, default=Path("lebron.mp3"),
    ...
    try:
        ...
        # in_fpath = "lebron.mp3"
        # embed1 = embed_mp3(in_fpath)
        # in_fpath2 = "obama.mp3"
        # embed2 = embed_mp3(in_fpath2)
        # # Interpolate between the embeddings
        # interpolation_weight = 0
        # embed = embed1 * (1 - interpolation_weight) + embed2 * interpolation_weight
        # embed /= np.linalg.norm(embed)

        path_files = [str(args.audio_path)]
        for i in range(len(path_files)):
            in_fpath = path_files[i]

            embed = embed_mp3(in_fpath)
